...
function sortFiles(files) {
  var sorted = _.sortBy(files, function (o) {
...
function createFinalDataJsonFiles(appPath) {
  ...
  try {
    (function () {
      var gameData = void 0,
          gameDataInCamelCase = void 0;
      var dataFiles = glob.sync(path.join(appPath, "components/**/data.json"));

      var files = sortFiles(dataFiles);
      ...
      writeFile(path.join(appPath, 'data', 'game-data.json'), JSON.stringify(gameData, null, '\t'));
      writeFile(path.join(appPath, 'data', 'game-data-in-camel-case.json'), JSON.stringify(gameDataInCamelCase, null, '\t'));
    })();
  } catch (e) {
    console.error(e);
  }
...
function formatKeyAsCamelCase(collection) {
  return _.each(collection, function (o, key) {
    ...
    var oldValue = collection[ccKey];

    if (_.isObject(o)) {
      var formattedValue = formatKeyAsCamelCase(o);
...
var DataJsonOptimizer = function () {
  /**
   * Create new instance of DataJsonOptimizer
   * @param options
   * @constructor
   */
  function DataJsonOptimizer() {
    var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

    _classCallCheck(this, DataJsonOptimizer);
  }
  ...
  _createClass(DataJsonOptimizer, [{
    key: 'apply',
    value: function apply(compiler) {
      ...
      compiler.plugin('compile', function (params) {
        var appPath = params && params.normalModuleFactory.context;
        createFinalDataJsonFiles(appPath);
      });
    }
  }]);

  return DataJsonOptimizer;
}();

exports.default = DataJsonOptimizer;
